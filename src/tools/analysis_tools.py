    ...
    async def perform_pca_analysis(
        ...
        try:
            ...
            for i, component in enumerate(components):
                component_weights[f"PC{component}"] = dict(zip(symbols, pca.components_[i, :].tolist()))
        ...
        try:
            ...
            return {
                ...
                "cointegration_critical_values": {
                    f"{int(k*100)}%": float(v) for k, v in result.critical_values.items()
                },
    ...
    async def analyze_mean_reversion(
        ...
        try:
            ...
            for symbol in symbols:
                if symbol in df.columns:
                    ...
                    if len(col_data) > lookback_period:
                        ...
                        z_score_current = (current_price - mean_price) / std_price if std_price > 0 else 0
                        ...
                        results[symbol] = {
                            ...
                            "signal_strength": abs(float(z_score_current)) if z_score_current < -1 else 0
                        }
            ...
            return {
                ...
                "total_signals": sum(1 for r in results.values() if r.get("is_mean_reversion_signal", False))
            }
        ...
        try:
            ...
            for i in range(len(symbols)):
                for j in range(i+1, len(symbols)):
                    if symbols[i] in correlation.columns and symbols[j] in correlation.columns:
            ...
            for symbol in symbols:
                if symbol in correlation.columns:
                    corr_sum = correlation[symbol].abs().sum() - 1  # Exclude self-correlation
                    asset_correlations[symbol] = {
                        "avg_abs_correlation": float(corr_sum / (len(symbols) - 1)),
                        "max_correlation": float(correlation[symbol].drop(symbol).max()),
                        "min_correlation": float(correlation[symbol].drop(symbol).min())
                    }
            ...
            return {
                ...
                "lowest_correlations": sorted(correlations_list, key=lambda x: abs(x["correlation"]))[:5],
