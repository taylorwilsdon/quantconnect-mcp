...
def register_universe_tools(mcp: FastMCP):
    ...
    async def add_etf_universe_securities(
        ...
        try:
            ...
            constituents_result = await get_etf_constituents(etf_ticker, date, instance_name)
        ...
        try:
            ...
            for symbol in symbols:
                if symbol in correlation.columns:
                    ...
                    corr_rank = correlation[symbol].abs().sum() - 1  # Subtract 1 for self-correlation
            ...
            if method == "lowest_correlation":
                ...
                selected_assets.sort(key=lambda x: x[1], reverse=True)  # Descending order
                title = "most correlated"
            
            # Select top N assets
            selected = selected_assets[:num_assets]
            benchmark = selected_assets[-num_assets:] if method == "lowest_correlation" else selected_assets[num_assets:2*num_assets]
            ...
            benchmark_symbols = [item[0] for item in benchmark] if len(benchmark) > 0 else []
            ...
            selected_performance = {
                ...
                "sharpe_ratio": float(selected_portfolio_return.mean() * 252 / (selected_portfolio_return.std() * np.sqrt(252))) if selected_portfolio_return.std() > 0 else 0
            }
            ...
            if benchmark_portfolio_return is not None:
                benchmark_performance = {
                    ...
                    "volatility": float(benchmark_portfolio_return.std() * np.sqrt(252)),
                    "sharpe_ratio": float(benchmark_portfolio_return.mean() * 252 / (benchmark_portfolio_return.std() * np.sqrt(252))) if benchmark_portfolio_return.std() > 0 else 0
                }
            ...
            return {
                ...
                "benchmark_assets": {
                    ...
                    "correlation_ranks": [item[1] for item in benchmark] if benchmark else [],
                    "avg_correlation": float(np.mean([item[1] for item in benchmark])) if benchmark else None,
        ...
        try:
            ...
            for i, symbol in enumerate(symbols):
                ...
                if benchmark_returns is not None:
                    correlation_with_benchmark = float(asset_returns.corr(benchmark_returns))
            ...
            for symbol, metrics in asset_metrics.items():
                ...
                if min_return is not None and metrics["annualized_return"] < min_return:
                    reasons_failed.append(f"Return {metrics['annualized_return']:.3f} < {min_return}")
                
                if max_volatility is not None and metrics["volatility"] > max_volatility:
                    reasons_failed.append(f"Volatility {metrics['volatility']:.3f} > {max_volatility}")
                
                if min_sharpe is not None and metrics["sharpe_ratio"] < min_sharpe:
                    reasons_failed.append(f"Sharpe {metrics['sharpe_ratio']:.3f} < {min_sharpe}")
                
                if max_correlation is not None and metrics["correlation_with_benchmark"] is not None:
                    if abs(metrics["correlation_with_benchmark"]) > max_correlation:
                        reasons_failed.append(f"Correlation {abs(metrics['correlation_with_benchmark']):.3f} > {max_correlation}")
