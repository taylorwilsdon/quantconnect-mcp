...
def register_portfolio_tools(mcp: FastMCP):
    ...
    async def sparse_optimization(
        ...
        try:
            ...
            pct_change_portfolio = np.log(portfolio_prices / portfolio_prices.shift(1)).dropna()
            pct_change_benchmark = np.log(benchmark_prices / benchmark_prices.shift(1)).loc[pct_change_portfolio.index]
            ...
            while iters < max_iter:
                ...
                q3 = 1/max_eig * (
                    ...
                    eig_vec @ d - 
                    2/m * pct_change_portfolio.T.values @ np.diagflat(a.T) @ 
            ...
            return {
                ...
                "benchmark_return": float((1 + pct_change_benchmark).cumprod().iloc[-1] - 1),
                "active_return": float(portfolio_returns.sum() - pct_change_benchmark.sum()),
                "optimization_params": {
    ...
    async def calculate_portfolio_performance(
        ...
        try:
            ...
            sortino_ratio = float(annualized_return / downside_deviation) if downside_deviation > 0 else 0
            ...
            if benchmark_symbol:
                ...
                information_ratio = float(active_returns.mean() * 252 / tracking_error) if tracking_error > 0 else 0
        ...
        try:
            ...
            for i, symbol in enumerate(symbols):
                ...
                asset_stats[symbol] = {
                    ...
                    "contribution_to_return": float(equal_weights[i] * asset_returns.sum())
                }
            ...
            return {
                ...
                "diversification_ratio": float(
                    np.sum(equal_weights * np.array([asset_stats[s]["volatility"] for s in symbols])) / volatility
